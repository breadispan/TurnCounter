...
function ETC.OnTurnStart(_, isPlayer)
    if isPlayer then return end  -- ❌ Ahora ignoramos el turno del oponente
...
function ETC.OnGameStart()
    ETC.currentTurn = 0 -- Reset turn count to 0 at the start of a match
...
function ETC.Initialize(event, addonName)
    ...
    --EVENT_MANAGER:RegisterForEvent("ETC_GameStart", EVENT_TRIBUTE_PLAYER_TURN_STARTED, ETC.OnGameStart)
    ...
    --EVENT_MANAGER:RegisterForEvent("ETC_TurnStart", EVENT_TRIBUTE_PLAYER_TURN_BEGINS, ETC.OnTurnStart)
...
function ETC.OnGameFlowStateChange(_, flowState)
    if flowState == TRIBUTE_GAME_FLOW_STATE_PLAYING then
        ETC.currentTurn = 0
        ETC.UpdateTurnDisplay()
        ETC_TURN_WINDOW:SetHidden(false)  -- ✅ Mostrar el contador al inicio de la partida
        d("[ExoYsTurnCounter] Game started, turn counter visible.")
    elseif flowState == TRIBUTE_GAME_FLOW_STATE_GAME_OVER then
        ETC_TURN_WINDOW:SetHidden(true)  -- ❌ Ocultar el contador cuando termine la partida
...
function ETC.CreateUI()
    ...
    -- ✅ Prevent duplicate creation: If the window already exists, do nothing
    ...
    ui:SetMovable(true) -- ✅ Allow the window to be moved
    ui:SetMouseEnabled(true) -- ✅ Allow mouse interactions (clicks, drags)
    ...
    bg:SetEdgeTexture("", 1, 1, 1) -- Simple border texture
